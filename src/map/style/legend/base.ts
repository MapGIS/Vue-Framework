...
export class LegendItem {
    ...
    constructor(options?) {
        options = options || {};
        const { contentType, height, width, imageData, label, url, values } = options;
    ...
    drawLine(line) {
        let {
            stroke = '#000000', strokeWidth = 8, strokeLineJoin = "round",
            width = 24, height = 24
        } = line;
        ...
        let base64 = canvas.toDataURL("image/png", 1);
        ctx.clearRect(0, 0, canvas.width - 2 * strokeWidth, canvas.height - 2 * strokeWidth);
        return base64;
    }
...
export class Legend {
    ...
    constructor(options?) {
        options = options || {};
        const { layerId, layerName, layerType, legend, maxScale, minScale } = options;
        this.layerId = layerId || 'unkown';
        this.layerName = layerName || 'unkown';
        this.layerType = layerType || LayerType.UnKnow;
        this.legend = legend || [];
        this.minScale = minScale || 0;
        this.maxScale = maxScale || 99999999;
    }

    addLegend(item: LegendItem) {
