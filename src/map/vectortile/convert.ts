...
import { defaultSources, addSource } from '../source/source'
import { LayerType, VectorTileType, VectorTileLayerDefine } from '../layer'
 ...
 * @param doc 地图文档
 */
export function docTomvt(doc: IDocument) {
    ...
    if (!doc) {
        ...
        style.sources = docTomvtSources(doc)
        style.layers = docTomvtLayers(doc)
    }
    return style
}

export function docTomvtSources(doc: IDocument) {
    // tslint:disable-next-line: prefer-const
    let sources = {}
    if (!doc) {
        return sources
...
export function docTomvtLayers(doc: IDocument) {
    // tslint:disable-next-line: prefer-const
    let layers = []
    if (!doc) {
        return layers
    ...
    layers = flats.map((layer) => {
        ...
        layer.set('type', docTomvtType(subtype))
        layer.set('layout', docTomvtLayout(layout))
...
export function docTomvtType(subtype) {
    return VectorTileLayerDefine[subtype].type
}

export function docTomvtLayout(layout) {
    if (!layout) {
 ...
 * @param doc
 */
export function mvtTodoc(mvt): IDocument {
    // let name = mvt.name || "地图样式";
    const sources = mvtTodocSources(mvt.sources);
    const layers = mvtTodocLayers(mvt.layers);
...
export function mvtTodocSources(sources) {
    if (!sources) {
    ...
    for (let index = 0; index < keys.length; index++) {
        ...
        if (source.type !== 'vector') {
            ...
            delete source.min;
            delete source.max;
            news = addSource(news, source, key);
        }
    }
...
export function mvtTodocLayers(layers) {
    let flats = []
    flats = layers.filter((layer) => {
        ...
        if (GeometryTypes.indexOf(layer.get('type')) < 0) {
            ...
            layer.set('subtype', mvtTodocType(type))
            ...
            layer.set('layout', mvtTodocLayout(layout) || {})
...
export function mvtTodocType(type) {
...
export function mvtTodocLayout(layout) {
    if (!layout) {
        layout = { visible: true }
    }
...
export function convertInspectLayers(layers, inspectId) {
    ...
    const unselectFill = convertUnselectLayer(fills, 0.8, 1.0)
    const unselectLine = convertUnselectLayer(lines, 0.7, 0.9)
    const unselectCircle = convertUnselectLayer(circles, 0.2, 0.4)
    const unselectSymbol = convertUnselectLayer(symbols, 0.6, 0.8)

    select = convertSelectlayer(select)
    return [].concat(unselectFill).concat(unselectLine).concat(unselectCircle).concat(unselectSymbol).concat(select)
}

export function convertUnselectLayer(layers, startPercent, endPercent) {
    const count = layers.length
    // tslint:disable-next-line: prefer-const
    let inspects = layers.map((layer, index) => {
        const percent = startPercent + (index / count) * (endPercent - startPercent)
        const color = d3.color(d3.interpolateGreys(percent)).copy({ opacity: 0.5 }).toString()
        if (layer.paint) {
...
export function convertSelectlayer(layer) {
    if (!layer) {
        layer = {
            filter: ['all'],
            paint: {}
...
export function convertInspectMode(doc: IDocument) {
    const origins = docTomvtLayers(doc)
    const inpsectid = doc.getCurrent().id
    // tslint:disable-next-line: prefer-const
    let inspects = convertInspectLayers(origins, inpsectid);
    return inspects;
}
