...
export class IDocument {
    ...
    addLayerNearLayer(layer: ILayer, where: string, before: boolean) {
        if (!where) {
            ...
            let children = this.layers.map(child => { return child; });
        ...
        this.layers = this.layers.map((layer, index) => {
            if (layer.type == LayerType.GroupLayer) {
                ...
                if (layer.id == id) {
                    ...
                    let special = type;
                    copy = deepCopy(layer);
                    copy = { ...copy, key: key, id: key, name: name, title: name, special: special, icon: iconfont };
                } else {
...
export const defaultSprites = "http://localhost:6163/igs/rest/mrms/vtiles/sprite";
